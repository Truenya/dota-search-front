...
export const CheckboxInput3 = (props: FIXMELATER) => {
    ...
    return (
        <FormControlLabel onChange={props.onChange} name={props.name} key={key}
                          control={<Checkbox defaultChecked icon={<DoNotDisturbAltIcon/>}
                                             checkedIcon={PosToIcon[props.name]}/>} label={props.childrn}/>
    );
};
...
export function CheckboxesFromPositions(positions: Positions, onChange: (e: { target: { type: string; name: any; checked: any; value: ((prevState: number[]) => number[]) | number[] } }) => void) {
    return (el: React.Key) => <CheckboxInput3 name={el} value={positions[el]} onChange={onChange} childrn={el}
                                              key={CheckBoxKeys[el]}/>;
}
...
export function CheckboxesFromPositionsNoText(positions: Positions, onChange: (e: { target: { type: string; name: any; checked: any; value: ((prevState: number[]) => number[]) | number[] } }) => void) {
    return (el: React.Key) => <CheckboxInput3 name={el} value={positions[el]} onChange={onChange} childrn={""}
                                              key={CheckBoxKeys[el]}/>;
}